...
const float POWER_HEATER = 5.5;
const float POWER_PUMP_CIRCULATION = 0.0;
const float POWER_PUMP1_LOW = 0.5;
const float POWER_PUMP1_HIGH = 3.0;
const float POWER_PUMP2_LOW = 0.45;
const float POWER_PUMP2_HIGH = 3.0;

const int MINUTES_PER_DEGC =
    12;  // Tweak for your tub - would be nice to auto-learn in the future to allow for outside temp etc
...
HASensor statusMessage("status_message");
HASensor timeMessage("time_message");
HASelect tubMode("modeselect");
...
HAButton btnTime("time");
HAButton btnEmpty("empty");

HAHVAC hvac(  // Not really HVAC device, but only way to get controls to set
    "temp", HAHVAC::TargetTemperatureFeature);
...
ArduinoQueue<String> sendBuffer(14);  // TODO: might be better bigger for large temp changes. Would need testing

void sendCommand(String command, int count) {
    telnetSend("Sending: " + command + String(count) + "times");
...
void onModeSwitchStateChanged(int8_t index, HASelect* sender) {
    telnetSend("Mode Switch changed: " + String(index));
    int currentIndex = sender->getCurrentState();
    int options = 3;
    sendCommand(COMMAND_CHANGE_MODE, 1);
...
void setup() {
    ...
    timeMessage.setName("Time Message");

    tubMode.setName("ModeSelect");
...
bool timeMessageIncoming = false;
...
void handleMessage() {
    ...
    if (result.substring(0, 4) == "fa14") {
        ...
        if (result.substring(10, 12) == "46" || result.substring(10, 12) == "2d") {
            statusMessage.setValue(result.c_str());
            ...
            if (newRaw != "") {
                ...
                if (!lastRaw3.equals(cmd)) {
                    ...
                    // sendBuffer.dequeue();
                    // telnetSend("YAY: command response: " + String(delayTime));
                }
                ...
                if (result.substring(10, 12) == "46") {  // "C"
        ...
        } else if (timeMessageIncoming) {
            String tempType = result.substring(10, 12);
            timeString = HexString2TimeString(result.substring(4, 12));
            telnetSend("Time: " + timeString);
            haTime.setValue(timeString.c_str());
            timeMessage.setValue(result.c_str());
...
void sendCommand() {
    if (sendBuffer.getHead() == COMMAND_EMPTY) {
        sendBuffer.dequeue();
        return;
    }
    if (!sendBuffer.isEmpty()) {
        ...
        if (digitalRead(PIN_5_PIN) != LOW) {
            telnetSend("ERROR: Pin5 went high before command before flush: " + String(delayTime));
            delayTime = 0;
            sendBuffer.dequeue();
        }
        // tub.flush(true);
        if (digitalRead(PIN_5_PIN) == LOW) {
            ...
            telnetSend("message sent: " + String(delayTime));
            sendBuffer.dequeue();
            // delayTime += 10;
        }
        // else {
...
String HexString2TimeString(String hexstring) {
    String time = "";
    time.concat(HexString2ASCIIString(hexstring.substring(0, 4)));
    time.concat(":");
    time.concat(HexString2ASCIIString(hexstring.substring(4, 8)));
    return time;
}
